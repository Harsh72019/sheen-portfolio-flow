...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      {/* Top Emerald Laser Scroll Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-500 origin-left z-[60]"
        style={{ scaleX }}
      />
      ...
      >
        <div className="container mx-auto px-3 sm:px-4 max-w-6xl">
          <nav className="glass-panel px-3.5 sm:px-6 py-2 sm:py-2.5 rounded-full flex items-center justify-between shadow-2xl border border-white/10 backdrop-blur-xl bg-[#0c141c]/85">
            {/* Brand Logo */}
            <a
              href="#hero"
              className="flex items-center gap-2 group cursor-pointer"
            >
              <div className="px-2.5 py-1 rounded-lg bg-white/[0.06] border border-white/10 text-slate-100 font-mono font-semibold text-xs group-hover:border-emerald-400/50 group-hover:text-white transition-colors">
            ...
            <div className="hidden md:flex items-center gap-2.5">
              ...
              <a
                href="#contact"
                className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-slate-950 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 rounded-full transition-all shadow-md shadow-emerald-500/20"
              >
            ...
            <div className="flex items-center gap-2 md:hidden">
              <a
                href="#contact"
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-950 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full transition-all shadow-sm"
              >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-4 top-20 z-40 md:hidden p-5 rounded-2xl glass-panel border border-white/15 bg-[#0c141c]/95 backdrop-blur-2xl shadow-2xl space-y-4"
          >
            ...
            <div className="pt-3 border-t border-white/10 flex flex-col gap-2">
              ...
              <a
                href="#contact"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-semibold text-slate-950 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-xl"
              >
      ...
      <AnimatePresence>
        {scrolled && activeSection !== "contact" && !isNearBottom && (
          <motion.a
            ...
            className="fixed bottom-5 right-4 z-40 md:hidden flex items-center gap-2 px-4 py-2.5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 text-xs font-semibold shadow-2xl border border-emerald-400/40 active:scale-95 transition-transform"
